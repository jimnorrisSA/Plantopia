...
function makeTemplateGrid(t) {
  ...
  t.pixels.forEach(([r, c], i) => {
    if (r < GRID && c < GRID) g[r][c] = i % 3 === 0 ? c2 : t.color;
  });
  return g;
}
...
function floodFill(grid, r, c, color) {
  ...
  while (stack.length) {
    ...
    if (cr < 0 || cr >= GRID || cc < 0 || cc >= GRID || next[cr][cc] !== target)
      continue;
    next[cr][cc] = color;
...
// Generate a short friend code from user id
...
function getCooldownKey(pinId) {
  return "plantopia_cd_" + pinId;
}
function getCooldownRemaining(pinId) {
  try {
    const val = localStorage.getItem(getCooldownKey(pinId));
    if (!val) return 0;
    return Math.max(0, parseInt(val) - Date.now());
  } catch (e) {
    return 0;
  }
}
function setCooldown(pinId, rarity) {
  const duration = COOLDOWNS[rarity] || COOLDOWNS.rare;
  localStorage.setItem(getCooldownKey(pinId), String(Date.now() + duration));
}
...
async function fetchLandmarks(lng, lat) {
  const url =
    ...
    "," +
    lat +
    ".json?types=poi&limit=10&access_token=" +
    mapboxgl.accessToken;
  try {
...
function generateCommonSeeds(lat, lng, count) {
  return Array.from({ length: count }, (_, i) => {
    ...
    const dist = 0.0002 + Math.random() * 0.0003; // keep within ~50m
    const tmpl =
      PLANT_TEMPLATES[Math.floor(Math.random() * PLANT_TEMPLATES.length)];
    return {
      id: "common-" + lat.toFixed(4) + "-" + i,
      name: tmpl.name + " Seed",
...
function getUnfurlOrder(grid) {
  ...
  for (let r = 0; r < GRID; r++)
    for (let c = 0; c < GRID; c++)
      if (grid[r][c]) p.push({ r, c, color: grid[r][c] });
  return p.sort((a, b) =>
    b.r !== a.r
      ? b.r - a.r
      : Math.abs(a.c - GRID / 2) - Math.abs(b.c - GRID / 2)
  );
}
...
function GrowingPlant({ grid, size, autoPlay }) {
  ...
  useEffect(() => {
    ...
    const step = () => {
      ...
      setVc(i);
      if (i < pixels.length) {
        tr.current = setTimeout(step, Math.max(8, 40 - i * 0.5));
      } else {
        setGrowing(false);
      }
    };
  ...
  const sz = size / GRID;
  const vis = new Set(pixels.slice(0, vc).map((p) => p.r + "-" + p.c));
  return (
    <div
      style={{
        width: size,
        ...
        return (
          <div
            ...
            style={{
              ...
              transition: v
                ? "opacity 0.12s, transform 0.15s cubic-bezier(0.34,1.56,0.64,1)"
                : "none",
            }}
          />
        );
      ...
      >
        ...
        {stage === "done" && (
          <div style={{ fontSize: 12, color: T.sub, textAlign: "center" }}>
            📍 Planted for the whole world to see!
          </div>
        )}
        <div style={{ display: "flex", gap: 6 }}>
          {["soil", "growing", "done"].map((s) => (
            <div
              key={s}
              style={{
                width: 6,
                height: 6,
                borderRadius: "50%",
                background: stage === s ? T.accent : T.border,
        ...
        {!collected ? (
          <>
            <div style={{ textAlign: "center", marginBottom: 20 }}>
              ...
              {isRare && (
                ...
                >
                  ✨ RARE — found at {pin.landmark_name || "a landmark"}
                </div>
              )}
              <div style={{ fontSize: 13, color: T.sub, marginTop: 8 }}>
                {isRare
                  ? "A rare seed from a special location!"
                  : "A wild seed waiting to be grown!"}
              </div>
            </div>
            <button
              ...
              style={{
                ...
                background: isRare
                  ...
                  : "linear-gradient(135deg," + T.yellow + "," + T.accent + ")",
                color: "#fff",
              }}
            >
              {isRare ? "🌟 Collect Rare Seed!" : "🌰 Collect Seed!"}
            </button>
            <button
              onClick={onClose}
              style={{
          ...
          <>
            <div style={{ textAlign: "center", padding: "12px 0 24px" }}>
              ...
              <div style={{ fontWeight: 800, fontSize: 20, color: T.text }}>
                {isRare ? "Rare Seed Collected!" : "Seed Collected!"}
              </div>
              <div style={{ fontSize: 13, color: T.sub, marginTop: 8 }}>
            ...
            <button
              ...
              style={{
                ...
                fontWeight: 800,
                fontSize: 16,
                background: "linear-gradient(135deg," + T.green + ",#20c997)",
                color: "#fff",
              }}
            >
              🌿 Go to Garden
...
function VisitModal({ pin, onHarvest, onClose, onHeart, isFriend }) {
  ...
  const [cdRemaining, setCdRemaining] = useState(() =>
    getCooldownRemaining(pin.id)
  );
  const cdRef = useRef(null);
  useEffect(() => {
    if (cdRemaining <= 0) return;
    cdRef.current = setInterval(() => {
      const rem = getCooldownRemaining(pin.id);
      setCdRemaining(rem);
  ...
  const tryHarvest = () => {
    ...
    setTimeout(() => {
      ...
      if (success) {
        ...
        setCooldown(pin.id, rarity);
        setCdRemaining(getCooldownRemaining(pin.id));
        setHarvested("no");
      }
    }, 900);
  };
      ...
      >
        {spark ? (
          <div style={{ textAlign: "center", padding: "32px 0" }}>
            ...
            <div style={{ fontWeight: 700, color: T.sub, marginTop: 12 }}>
              Checking for seeds…
            </div>
          </div>
        ) : harvested === "yes" ? (
          <>
            <div style={{ textAlign: "center", padding: "8px 0 16px" }}>
              ...
              <div style={{ fontSize: 13, color: T.sub, margin: "8px 0 20px" }}>
                You got a cutting of <b>{pin.name}</b>!
              </div>
            </div>
            <button
              onClick={() => onHarvest(pin)}
              style={{
                width: "100%",
                padding: 14,
                borderRadius: 14,
          ...
          <>
            <div style={{ textAlign: "center", padding: "8px 0 16px" }}>
              ...
              <div style={{ fontSize: 13, color: T.sub, margin: "8px 0 12px" }}>
                This plant needs time to recover.
              </div>
              <div
                style={{
                  background: "#fff5f5",
              ...
              >
                ...
                <span
                  style={{ fontWeight: 700, fontSize: 14, color: T.accent }}
                >
                  Try again in {formatCooldown(getCooldownRemaining(pin.id))}
                </span>
              </div>
            </div>
            <button
          ...
          <>
            <div style={{ textAlign: "center", marginBottom: 20 }}>
              ...
              >
                <GrowingPlant grid={pin.grid} size={72} />
              </div>
              <div style={{ fontWeight: 800, fontSize: 20, color: T.text }}>
                {pin.name}
              </div>
              <div style={{ fontSize: 13, color: T.sub, marginTop: 4 }}>
                By <b style={{ color: T.text }}>{pin.username}</b> · {pin.distM}
                m away
              </div>
              <div style={{ fontSize: 13, color: T.accent, marginTop: 4 }}>
                💚 {pin.hearts || 0} hearts
              </div>
            </div>
            {!pin.harvested &&
              (onCooldown ? (
                ...
                >
                  ...
                  <div>
                    <div
                      style={{ fontWeight: 700, fontSize: 13, color: T.accent }}
                    >
                      Harvest on cooldown
                    </div>
                    <div style={{ fontSize: 12, color: T.sub }}>
                      Try again in <b>{cdLabel}</b>
                    </div>
                  </div>
                </div>
              ) : (
...
// ─── Pot slot component ───────────────────────────────────────────────────────
function PotSlot({ plant, onPlaceInWorld, onRemove, slotIndex }) {
  ...
  return (
    <div
      style={{
        ...
        padding: 14,
        boxShadow: T.shadow,
        border: "2px solid " + (isRare ? "#f59e0b" : seed.color + "44"),
      }}
    >
      <div
        style={{
          background: isRare ? "#fef9ee" : "#f9f9f9",
          borderRadius: 12,
          padding: 8,
          marginBottom: 10,
          display: "flex",
      ...
      >
        {seed.grid ? (
          <GrowingPlant grid={seed.grid} size={64} />
        ) : (
      ...
      <div
        style={{
          fontSize: 10,
          color: isRare ? "#f59e0b" : seed.color,
          fontWeight: 700,
          marginBottom: 8,
        }}
      >
      ...
      <button
        ...
        style={{
          ...
          background: isRare
            ? "linear-gradient(135deg,#f59e0b,#ec4899)"
            : "linear-gradient(135deg," + T.green + ",#20c997)",
          color: "#fff",
        }}
      >
        🌱 Plant on Map
  ...
  return (
    <div
      style={{
        ...
        fontSize: 11,
        fontWeight: 700,
        color: ok ? "#51cf66" : gps ? "#ffd43b" : "#ff6b6b",
        background: ok ? "#f0fff4" : gps ? "#fffbeb" : "#fff5f5",
        borderRadius: 20,
        padding: "3px 10px",
        border: "1px solid " + (ok ? "#b2f2bb" : gps ? "#fde68a" : "#fecaca"),
      }}
    >
      <div
        style={{
          ...
          height: 6,
          borderRadius: "50%",
          background: ok ? "#51cf66" : gps ? "#ffd43b" : "#ff6b6b",
          boxShadow: ok ? "0 0 6px #51cf66" : "none",
        }}
      />
...
function UsernameScreen({ onSet }) {
  ...
  return (
    ...
    >
      ...
      >
        Plant pixel art in the real world.
        <br />
      ...
      <div style={{ width: "100%", maxWidth: 320 }}>
        <input
          ...
          style={{
            ...
            background: T.card,
            border: "2px solid " + (error ? T.accent : T.border),
            borderRadius: 16,
            padding: "14px 18px",
            color: T.text,
            fontSize: 16,
        ...
        <button
          onClick={submit}
          disabled={loading || val.trim().length < 3}
          style={{
            width: "100%",
            padding: 16,
            borderRadius: 16,
            ...
            background: "linear-gradient(135deg," + T.green + ",#20c997)",
            color: "#fff",
            opacity: loading || val.trim().length < 3 ? 0.6 : 1,
          }}
        >
...
function FriendsScreen({ user, worldPlants, onClose, onHarvest, onHeart }) {
  ...
  const addFriend = async () => {
    ...
    if (code.length !== 6) {
      setAddError("Enter a 6-character friend code");
      return;
    }
    setAddLoading(true);
    setAddError(null);
    ...
    if (!target) {
      setAddError("No player found with that code!");
      setAddLoading(false);
      return;
    }
    ...
    if (error) {
      setAddError("Couldn't add friend.");
      setAddLoading(false);
      return;
    }
    setAddCode("");
  ...
  const removeFriend = async (friendId) => {
    ...
    if (selectedFriend && selectedFriend.friend_id === friendId)
      setSelectedFriend(null);
  };

  const friendPlants = selectedFriend
    ? worldPlants.filter((p) => p.username === selectedFriend.friend_username)
    : [];
  const featuredPlant = friendPlants[0] || null;
      ...
      <div style={{ padding: 16, maxWidth: 430, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input
              ...
              style={{
                flex: 1,
                background: "#f9f9f9",
                border: "2px solid " + (addError ? T.accent : T.border),
                borderRadius: 12,
                padding: "10px 14px",
                color: T.text,
                fontSize: 16,
            ...
            <button
              ...
              style={{
                ...
                background: "linear-gradient(135deg," + T.green + ",#20c997)",
                color: "#fff",
                opacity: addLoading || addCode.length !== 6 ? 0.5 : 1,
              }}
            >
              {addLoading ? "…" : "Add"}
            </button>
        ...
        {loading ? (
          ...
          <>
            ...
            {friends.map((f) => {
              const fPlants = worldPlants.filter(
                (p) => p.username === f.friend_username
              );
              const fFeatured = fPlants[0];
              const isSelected =
                selectedFriend && selectedFriend.friend_id === f.friend_id;
              return (
                <div
                  key={f.friend_id}
                  style={{
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: T.sub }}>
                        {fPlants.length} plant{fPlants.length !== 1 ? "s" : ""}{" "}
                        in world
                      </div>
                    </div>
    ...
    >
      ...
      <div style={{ padding: 16, maxWidth: 430, margin: "0 auto" }}>
        ...
        >
          ...
          <div style={{ fontWeight: 900, fontSize: 22, color: T.text }}>
            @{user.username}
          </div>
          <div
            style={{
          ...
          >
            {[
              { val: myPlants.length, label: "Plants" },
              { val: totalHearts, label: "Hearts" },
              { val: inventory.length, label: "Seeds" },
              { val: potsUsed + "/" + MAX_POTS, label: "Pots" },
            ].map(({ val, label }) => (
          ...
          >
            {BADGES.map((b) => (
              <div
                key={b.label}
                style={{
                  background: b.earned ? "#f0fff4" : "#f9f9f9",
                  borderRadius: 14,
                  padding: "12px 8px",
                  textAlign: "center",
                  border: "2px solid " + (b.earned ? "#b2f2bb" : T.border),
...
function MapView({
  ...
  useEffect(() => {
    ...
    if (!map.current.isStyleLoaded()) map.current.once("load", centre);
    else centre();
  ...
  useEffect(() => {
    ...
    if (!userMarker.current) {
      ...
      el.style.cssText =
        ...
        T.accent +
        ";border:3px solid white;display:flex;align-items:center;justify-content:center;font-size:14px;box-shadow:0 0 0 6px " +
        T.accent +
        "33;";
      el.innerHTML = "😊";
  ...
  useEffect(() => {
    ...
    if (!canvas) return;
    canvas.style.cursor = placingPlant ? "crosshair" : "";
  }, [placingPlant]);

  useEffect(() => {
    if (!map.current) return;
    const allPins = [
      ...nearbyPlants.map((p) => ({ ...p, pinType: "plant" })),
      ...nearbySeeds.map((s) => ({ ...s, pinType: "dbseed" })),
      ...localSeeds.map((s) => ({ ...s, pinType: "localseed" })),
    ];
    const doRender = () => {
      ...
      allPins.forEach((pin) => {
        ...
        const el = document.createElement("div");
        const isRare = pin.rarity === "rare";
        const isSeed = pin.pinType !== "plant";
        if (isSeed) {
          el.style.cssText =
            "width:" +
            (isRare ? 36 : 28) +
            ...
            (isRare ? "#f59e0b" : pin.color) +
            ";border:3px solid white;display:flex;align-items:center;justify-content:center;font-size:" +
            (isRare ? 18 : 14) +
            "px;box-shadow:" +
            (isRare ? "0 0 12px #f59e0b88" : "0 2px 8px rgba(0,0,0,0.2)") +
            ";cursor:pointer;";
          el.innerHTML = isRare ? "🌟" : "🌰";
        } else {
          ...
          el.style.cssText =
            "width:36px;height:36px;border-radius:8px;border:2px solid white;overflow:hidden;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,0.25);image-rendering:pixelated;background:#f9f9f9;";
    ...
    if (!map.current.isStyleLoaded()) map.current.once("load", doRender);
    else doRender();
  ...
  return (
    <div
      style={{
        ...
        overflow: "hidden",
        boxShadow: T.shadow,
        border: "2px solid " + (placingPlant ? T.accent : T.border),
    ...
    >
      ...
      {placingPlant && (
        ...
        >
          📍 Tap map to plant "{placingPlant.name}"
        </div>
      )}
      {gpsAccuracy && userPos && (
        <div
...
export default function App() {
  ...
  // pots: array of MAX_POTS slots, each is a plant object or null
  const [pots, setPots] = useState(() => {
    try {
      const s = localStorage.getItem("plantopia_pots");
      if (s) {
        const arr = JSON.parse(s);
        // ensure length matches MAX_POTS
        return Array(MAX_POTS)
          .fill(null)
          .map((_, i) => arr[i] || null);
      }
    } catch (e) {}
    // migrate from old "garden"
    try {
      const s = localStorage.getItem("plantopia_garden");
      if (s) {
  ...
  const [inventory, setInventory] = useState(() => {
    try {
      const s = localStorage.getItem("plantopia_inventory");
  ...
  useEffect(() => {
    const stored = localStorage.getItem("plantopia_user");
    if (stored) {
      try {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("plantopia_pots", JSON.stringify(pots));
    } catch (e) {}
  }, [pots]);

  useEffect(() => {
    try {
      localStorage.setItem("plantopia_inventory", JSON.stringify(inventory));
    } catch (e) {}
  }, [inventory]);
  ...
  useEffect(() => {
    const upd = () => {
      if (containerRef.current) {
        ...
        setCs(Math.floor(Math.min(w, 340) / GRID));
      }
    };
    upd();
  ...
  useEffect(() => {
    ...
    watchRef.current = navigator.geolocation.watchPosition(
      ...
      (err) => {
        setGpsError(
          err.code === 1 ? "Location denied." : "Unable to get location."
        );
        setGpsLoading(false);
      },
  ...
  useEffect(() => {
    ...
    if (!landmarksLoadedRef.current) {
      landmarksLoadedRef.current = true;
      loadLandmarkAndCommonSeeds(userPos.lat, userPos.lng);
    }
  ...
  const loadLandmarkAndCommonSeeds = async (lat, lng) => {
    ...
    const rare = landmarks.slice(0, 4).map((lm, i) => {
      const tmpl = RARE_TEMPLATES[i % RARE_TEMPLATES.length];
      return {
        id: "rare-" + lm.id,
        name: tmpl.name + " Seed",
        lat: lm.center[1],
  ...
  const nearbyPlants = userPos
    ? worldPlants.filter(
        (p) =>
          haversine(userPos.lat, userPos.lng, p.lat, p.lng) <= PROXIMITY_RADIUS
      )
    : [];
  const nearbySeeds = userPos
    ? worldSeeds.filter(
        (s) =>
          haversine(userPos.lat, userPos.lng, s.lat, s.lng) <= PROXIMITY_RADIUS
      )
    : [];
  const nearbyLocalSeeds = userPos
    ? localSeeds.filter(
        (s) =>
          haversine(userPos.lat, userPos.lng, s.lat, s.lng) <= PROXIMITY_RADIUS
      )
  ...
  const allNearby = userPos
    ? [
        ...nearbyPlants.map((p) => ({
          ...p,
          pinType: "plant",
          distM: haversine(userPos.lat, userPos.lng, p.lat, p.lng),
        })),
        ...nearbySeeds.map((s) => ({
          ...s,
          pinType: "dbseed",
          distM: haversine(userPos.lat, userPos.lng, s.lat, s.lng),
        })),
        ...nearbyLocalSeeds.map((s) => ({
          ...s,
          pinType: "localseed",
          distM: haversine(userPos.lat, userPos.lng, s.lat, s.lng),
        })),
  ...
  const paint = useCallback(
    (r, c) => {
      if (tool === "fill") {
        pushHistory(floodFill(grid, r, c, color));
        return;
      }
      const next = grid.map((row) => [...row]);
      next[r][c] = tool === "erase" ? null : color;
      setGrid(next);
    },
    [grid, color, tool, pushHistory]
  );

  const endStroke = useCallback(() => {
    if (tool !== "fill") pushHistory(grid);
    setPainting(false);
  }, [grid, tool, pushHistory]);

  const potsUsed = pots.filter(Boolean).length;
  const potsFull = potsUsed >= MAX_POTS;

  const savePlant = () => {
    if (!grid.some((row) => row.some((c) => c))) {
      showToast("Draw something first! 🖌️");
      return;
    }
    if (potsFull) {
      showToast("All pots are full! Remove one first 🪴");
      setTab("garden");
      return;
    }
    ...
    setPots((p) => {
      const idx = p.findIndex((slot) => !slot);
      if (idx === -1) return p;
      const n = [...p];
      n[idx] = newPlant;
      return n;
    ...
    setPlantName("");
    pushHistory(mkGrid());
    showToast('🌱 "' + name + '" saved to pot ' + (potsUsed + 1) + "!");
    setTimeout(() => setTab("garden"), 1500);
  };

  const removePlantFromPot = (plant) => {
    setPots((p) =>
      p.map((slot) => (slot && slot.id === plant.id ? null : slot))
    );
    ...
    if (plant.isSeed) {
      if (plant.random) {
        ...
        resolved = {
          ...plant,
          name: tmpl.name,
          grid: makeTemplateGrid(tmpl),
          color: tmpl.color,
          isSeed: false,
        };
      } else {
        resolved = { ...plant, isSeed: false };
      }
      setInventory((s) => s.filter((sd) => sd.id !== plant.id));
      if (plant.dbId)
        await supabase.from("seeds").delete().eq("id", plant.dbId);
    } else {
      // placing from pots — remove from pot when placed
      setPots((p) =>
        p.map((slot) => (slot && slot.id === plant.id ? null : slot))
      );
    }
  ...
  const handleMapClick = async (lat, lng) => {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        name: placingPlant.name,
        lat,
        lng,
        grid: placingPlant.grid,
        color: placingPlant.color,
      })
      .select()
      .single();
    if (error) {
      showToast("Couldn't plant — check connection");
      return;
    }
    setWorldPlants((p) => [...p, data]);
    setCeremony({ plant: { ...placingPlant } });
    setPlacingPlant(null);
    const sLat = lat + (Math.random() - 0.5) * 0.0003,
      sLng = lng + (Math.random() - 0.5) * 0.0003;
    await supabase
      .from("seeds")
      .insert({
        name: placingPlant.name + " Seed",
        lat: sLat,
        lng: sLng,
        color: placingPlant.color,
        is_random: false,
        source_plant_id: data.id,
      });
    await fetchNearby();
  };
  ...
  const handlePinClick = (pin) => {
    ...
    if (pin.pinType === "plant")
      setVisitModal({
        ...pin,
        distM: userPos
          ? haversine(userPos.lat, userPos.lng, pin.lat, pin.lng)
          : 0,
      });
      ...
      ...s,
      {
        ...
        name:
          pin.rarity === "rare"
            ...
            : pin.name + " Cutting",
        color: pin.color,
        random: !!pin.is_random,
        rarity: pin.rarity || "common",
        grid: null,
        isSeed: true,
        dbId: pin.local ? null : pin.id,
      },
    ]);
    if (pin.local) setLocalSeeds((s) => s.filter((ss) => ss.id !== pin.id));
    else {
      setWorldSeeds((s) => s.filter((ss) => ss.id !== pin.id));
      if (pin.id) supabase.from("seeds").delete().eq("id", pin.id);
    }
    setSeedModal(null);
    setTab("garden");
    showToast(
      pin.rarity === "rare" ? "🌟 Rare seed collected!" : "🌰 Seed collected!"
    );
  };
  ...
  const handleHarvest = (pin) => {
    ...
    setWorldPlants((p) =>
      p.map((pp) => (pp.id === pin.id ? { ...pp, harvested: true } : pp))
    );
    setVisitModal(null);
    setTab("garden");
    showToast("🌿 " + pin.name + " cutting added!");
  };

  const handleHeart = async (pin) => {
    await supabase
      .from("plants")
      .update({ hearts: (pin.hearts || 0) + 1 })
      .eq("id", pin.id);
    setWorldPlants((p) =>
      p.map((pp) =>
        pp.id === pin.id ? { ...pp, hearts: (pp.hearts || 0) + 1 } : pp
      )
    );
    ...
    >
      ...
      >
        <div style={{ fontSize: 16, fontWeight: 900, color: T.text }}>
          🌱 <span style={{ color: T.green }}>{user.username}</span>
        </div>
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          {inventory.length > 0 && (
            <span style={{ fontSize: 12, color: T.yellow, fontWeight: 700 }}>
              🌰×{inventory.length}
            </span>
          )}
          <span
            style={{
              fontSize: 12,
              color: potsUsed >= MAX_POTS ? T.accent : T.sub,
              fontWeight: 700,
            }}
          >
            🪴{potsUsed}/{MAX_POTS}
        ...
        {tab === "editor" && (
          <div style={{ padding: "16px 16px 0" }}>
            ...
            <div style={{ fontSize: 12, color: T.sub, marginBottom: 6 }}>
              Design your pixel plant to place in the world
            </div>
            {potsFull && (
              <div
                style={{
              ...
              >
                <span>🪴</span> All {MAX_POTS} pots are full — go to Garden to
                free up a spot before saving.
              </div>
            )}
            <div
              style={{
              ...
              {TOOLS.map((t) => (
                <button
                  ...
                  style={{
                    ...
                    fontSize: 20,
                    cursor: "pointer",
                    background: tool === t.id ? T.accent : "#f5f5f5",
                    boxShadow:
                      tool === t.id
                        ? "0 4px 12px " + T.accent + "55"
                        : T.shadow,
                    transform: tool === t.id ? "scale(1.1)" : "scale(1)",
                ...
                {Object.keys(PALETTES).map((p) => (
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 11,
                      fontWeight: 700,
                      background: palette === p ? T.accent : "#f5f5f5",
                      color: palette === p ? "#fff" : T.text,
                    }}
                  >
                    {p}
                  </button>
            ...
            >
              ...
              >
                <div
                  style={{
                    display: "grid",
                    gridTemplateColumns: "repeat(" + GRID + "," + cs + "px)",
                      ...
                      <div
                        ...
                        onTouchMove={(e) => {
                          ...
                          if (el && el.dataset.r !== undefined)
                            paint(+el.dataset.r, +el.dataset.c);
                        }}
                        data-r={r}
                        data-c={c}
                      />
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button
                ...
                style={{
                  ...
                  color: "#fff",
                  fontSize: 16,
                  boxShadow: potsFull ? "none" : "0 4px 16px " + T.green + "66",
                }}
              >
                🌱
              </button>
          ...
          <div style={{ padding: 16 }}>
            ...
            >
              {pots.map((plant, i) => (
                <PotSlot
                  ...
                  plant={plant}
                  slotIndex={i}
                  onPlaceInWorld={placeOnMap}
                  onRemove={removePlantFromPot}
                />
              ))}
            </div>
            ...
            {inventory.length > 0 && (
              <>
                ...
                >
                  {inventory.map((seed) => (
                    <SeedCard key={seed.id} seed={seed} onPlant={placeOnMap} />
                  ))}
                </div>
              </>
            )}
            ...
            {potsUsed === 0 && inventory.length === 0 && (
              ...
              >
                ...
                <div style={{ fontSize: 13, margin: "8px 0 20px" }}>
                  Draw one or collect seeds on the map.
                </div>
                <button
        ...
        {tab === "map" && (
          <div style={{ padding: 16 }}>
            ...
            <div
              style={{
                fontSize: 12,
                color: placingPlant ? T.accent : T.sub,
                marginBottom: 10,
                fontWeight: placingPlant ? 700 : 400,
              }}
            >
              {placingPlant
                ? "Tap the map to plant!"
                : "🌟 Rare seeds at landmarks · 🌰 Common seeds within " +
                  PROXIMITY_RADIUS +
                  "m"}
            </div>
            {gpsError && (
            ...
            <MapView
              ...
              nearbySeeds={nearbySeeds}
              localSeeds={nearbyLocalSeeds}
            ...
            {userPos && allNearby.length > 0 && (
              <>
                ...
                >
                  Within {PROXIMITY_RADIUS}m ({allNearby.length})
                </div>
                <div
                ...
                >
                  {allNearby.slice(0, 20).map((pin) => (
                    ...
                    >
                      ...
                      >
                        {pin.pinType === "plant" && pin.grid ? (
                          <GrowingPlant grid={pin.grid} size={32} />
                        ) : (
                        ...
                        <div style={{ fontSize: 11, color: T.sub }}>
                          {pin.pinType === "plant"
                            ? "By " +
                              ...
                              (pin.hearts ? " · 💚" + pin.hearts : "")
                            : pin.rarity === "rare"
                            ? "At " + (pin.landmark_name || "landmark")
                            : "Seed · tap to collect"}
                        </div>
                      </div>
                      <div
            ...
            {userPos && allNearby.length === 0 && (
              <div
                style={{ textAlign: "center", padding: "32px 0", color: T.sub }}
              >
                <div style={{ fontSize: 40, marginBottom: 8 }}>🌍</div>
                <div style={{ fontSize: 13 }}>
                  Nothing within {PROXIMITY_RADIUS}m.
                  <br />
                  Explore to discover plants and seeds!
                </div>
              </div>
            )}
          </div>
          ...
          >
            <div
              style={{
                fontSize: 22,
                lineHeight: 1,
                filter: tab === n.id ? "none" : "grayscale(0.3)",
                transform: tab === n.id ? "scale(1.18)" : "scale(1)",
                transition: "all 0.2s",
              }}
            >
              {n.icon}
